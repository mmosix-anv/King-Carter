...
export class StrapiClient {
  ...
  /**
   * Fetches navigation links from Strapi NavLinks single type
   * 
   * Returns navigation links data including leftLinks, rightLinks, and ctaButtons.
   * Falls back to default navigation structure if Strapi is unavailable.
   * 
   * @returns {Promise<Object>} Navigation links data
   * 
   * @example
   * const client = new StrapiClient();
   * const navLinks = await client.fetchNavLinks();
   * console.log(navLinks.leftLinks); // [{ label: 'Services', url: '/services', ... }]
   */
  async fetchNavLinks() {
    const fallbackData = {
      leftLinks: [
    ...
    try {
      const response = await this._makeRequest('/api/nav-links?populate=*');
      
      if (!response?.data) {
        this._log('warn', 'Invalid nav-links response, using fallback data');
        return fallbackData;
      }

      // Extract data from single type response
      const data = response.data;
      
      const navLinks = {
        leftLinks: data.leftLinks || fallbackData.leftLinks,
        rightLinks: data.rightLinks || fallbackData.rightLinks,
        ctaButtons: data.ctaButtons || fallbackData.ctaButtons
      };

      this._log('info', 'Successfully fetched navigation links from Strapi');
      return navLinks;

    } catch (error) {
      if (error.code === 'ECONNREFUSED' || error.code === 'ETIMEDOUT' || error.code === 'ENOTFOUND') {
        this._log('warn', 'Strapi unavailable for nav-links, using fallback data');
      } else if (error.response) {
        this._log('error', 'Strapi error for nav-links, using fallback data', {
          status: error.response.status,
          message: error.response.data?.error?.message || error.message
        });
      } else {
        this._log('error', 'Unexpected error fetching nav-links, using fallback data', {
          message: error.message
        });
      }
